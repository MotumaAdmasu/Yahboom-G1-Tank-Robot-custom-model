...
class TankController(Node):
    ...
    def cmd_vel_callback(self, msg):
        ...
        left_linear_vel = msg.linear.x - (msg.angular.z * wheel_separation / 2.0)
        right_linear_vel = msg.linear.x + (msg.angular.z * wheel_separation / 2.0)
        
        # Convert to angular velocities (rad/s)
        self.left_wheel_vel = left_linear_vel / wheel_radius
        self.right_wheel_vel = right_linear_vel / wheel_radius
        ...
        movement = self.get_movement_description(msg.linear.x, msg.angular.z)
        self.get_logger().info(f'🚀 {movement}')
        self.get_logger().info(f'   Linear: {msg.linear.x:.2f} m/s, Angular: {msg.angular.z:.2f} rad/s')
